...
function StatusTag({ status }: { status: string }) {
  ...
  const c = map[status] ?? { bg: Colors.surfaceAlt, text: Colors.textSecondary };
  return (
  ...
  return (
    <Animated.View entering={FadeInDown.delay(delay).springify().damping(14)}>
      <TouchableOpacity
        ...
        onPress={() => router.push({ pathname: '/(tabs)/insights', params: { reportId: report.reportId } })}
        activeOpacity={0.85}
      >
        ...
        <View style={styles.reportCardBody}>
          <Text style={styles.reportCardName} numberOfLines={1}>{report.fileName}</Text>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      >
        ...
        {lastReport ? (
          <Animated.View entering={FadeInDown.delay(80).springify().damping(14)}>
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: '/(tabs)/insights', params: { reportId: lastReport.reportId } })}
              activeOpacity={0.88}
            >
              ...
              <Text style={styles.summaryCardDate}>
                {new Date(lastReport.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'long' })}
              </Text>
              ...
              <View style={styles.summaryCardCta}>
                <Text style={styles.summaryCardCtaText}>View full analysis →</Text>
              </View>
            </TouchableOpacity>
          </Animated.View>
        ) : (
          <Animated.View entering={FadeInDown.delay(80).springify().damping(14)}>
            <View style={styles.emptyCard}>
              ...
              <Text style={styles.emptyCardSub}>Upload your first medical report to get started</Text>
            </View>
          </Animated.View>
        )}
        ...
        <Animated.View entering={FadeInDown.delay(200).springify().damping(14)} style={styles.actionsRow}>
          <TouchableOpacity
          ...
          {lastReport && (
            <TouchableOpacity
              ...
              onPress={() => router.push({ pathname: '/(tabs)/insights', params: { reportId: lastReport.reportId, chat: '1' } })}
              activeOpacity={0.87}
            >
        ...
        {reports.length > 0 && (
          <>
            <Animated.View entering={FadeInDown.delay(280).springify()} style={styles.sectionHeader}>
        ...
        {/* Disclaimer */}
        <Animated.View entering={FadeInDown.delay(500).springify()} style={styles.disclaimerBox}>
          <Text style={styles.disclaimerText}>
            ⚠️  MedLens does not provide medical advice. Always consult a qualified healthcare professional.
          </Text>
        </Animated.View>
...
const styles = StyleSheet.create({
  ...
  nameText: { fontSize: 30, fontWeight: '300', color: Colors.text, letterSpacing: -1, marginTop: 2 },
  avatarCircle: {
  ...
  summaryCard: {
    backgroundColor: Colors.dark, borderRadius: 24, padding: Spacing.lg,
    marginBottom: Spacing.xl, gap: 8, ...Shadow.md,
  },
  summaryCardTop: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  summaryCardLabel: { fontSize: 10, fontWeight: '700', letterSpacing: 1.2, color: 'rgba(255,255,255,0.4)' },
  summaryCardTitle: { fontSize: 20, fontWeight: '600', color: '#fff', letterSpacing: -0.5 },
  summaryCardDate: { fontSize: 12, color: 'rgba(255,255,255,0.45)' },
  summaryCardText: { fontSize: 14, color: 'rgba(255,255,255,0.7)', lineHeight: 20, marginTop: 4 },
  ...
  emptyCard: {
    ...
    alignItems: 'center', gap: 12, marginBottom: Spacing.xl, ...Shadow.sm,
    borderWidth: 2, borderColor: Colors.border, borderStyle: 'dashed',
  },
  emptyCardTitle: { fontSize: 17, fontWeight: '600', color: Colors.text },
  emptyCardSub: { fontSize: 14, color: Colors.textSecondary, textAlign: 'center', lineHeight: 20 },
  sectionHeader: {
    flexDirection: 'row', justifyContent: 'space-between',
    alignItems: 'center', marginBottom: 12,
  },
  sectionTitle: { fontSize: 11, fontWeight: '700', letterSpacing: 1.2, color: Colors.textSecondary },
  sectionLink: { fontSize: 13, color: Colors.accent, fontWeight: '600' },
  actionsRow: { flexDirection: 'row', gap: 12, marginBottom: Spacing.xl },
  actionCardPrimary: {
    flex: 1, backgroundColor: Colors.text, borderRadius: 20,
  ...
  actionCardPrimaryText: { fontSize: 15, fontWeight: '600', color: '#fff', marginTop: 4 },
  actionCardPrimarySub: { fontSize: 12, color: 'rgba(255,255,255,0.5)' },
  actionCardSecondary: {
    flex: 1, backgroundColor: Colors.surface, borderRadius: 20,
    padding: Spacing.lg, gap: 8, ...Shadow.sm,
    borderWidth: 1, borderColor: Colors.border,
  },
  actionCardSecondaryText: { fontSize: 15, fontWeight: '600', color: Colors.text, marginTop: 4 },
